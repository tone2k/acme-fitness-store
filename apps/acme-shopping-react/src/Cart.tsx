  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      renderCell: (params) => {
        const row = params.row as CartItemData;
        return (
          <Link to={`/product/${row.itemid}`} color="inherit">
            {row.name}
          </Link>
        );
      },
    ...
    },
    {
      ...
      renderCell: (params) => {
        return (
          <IconButton
            color="inherit"
            onClick={() => removeItemFromCart(params.row as CartItemData)}
          >
            <DeleteIcon />
          </IconButton>
        );
    ...
    <Container sx={{ mt: 2 }}>
      ...
      <Grid mt={5} container spacing={3} alignItems="flex-start">
        <Grid item xs={12} md={9}>
          <Typography sx={{ my: "2rem" }} align="center" variant="h3">
            Your Shopping cart awaits!
          </Typography>
          {cartItems.length > 0 ? (
            <>
              <Typography sx={{ my: "1rem" }}>
                You currently have <strong>{cartItems.length} item(s)</strong>{" "}
                in your cart
              </Typography>
              <DataGrid
